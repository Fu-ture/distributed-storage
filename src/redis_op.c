...
/* -------------------------------------------*/
/**
 * @brief                  创建或者覆盖一个string类型的key-value对
 *
 * @param conn             已经建立好的链接
 * @param key              key值
 * @param str              value值
...
int rop_set_String(redisContext *conn, char *key,char *str)
{
    ...
    if(strcmp(reply->str,"OK") != 0) {
        ...
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC,"set %s %s err",key ,str);
        retn = -1;
        goto END;
...
int rop_get_String(redisContext *conn, char *key,char *str)
{
    ...
    if(reply->type != REDIS_REPLY_STATUS) {
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC,"get %s err",key);
        retn = -1;
        goto END;
    }
 ...
 */
/* -------------------------------------------*/
int rop_hash_get(redisContext *conn, char *key, RFIELDS field , RVALUES value)
{
    int retn = 0;
    redisReply *reply = NULL;
    ...
    if (reply->type != REDIS_REPLY_ARRAY) {
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]HGET %s  error!%s\n", key, conn->errstr);
        retn = -1;
    }
...
int rop_range_list(redisContext *conn, char *key, int from_pos, int count, RVALUES values, int *get_num)
{
    ...
    LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "come in rop_range\n");
    int retn = 0;
    int i = 0;
    redisReply *reply = NULL;
    int max_count = 0;
    ...
    LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "lrange %d\n", reply->elements);
...
int rop_zset_increment(redisContext *conn, char* key, char* member)
{
    ...
    //rop_test_reply_type(reply);
    if (strcmp(reply->str, "OK") != 0) {
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]Add or increment table: %s,member: %s OK:%s,%s\n", key, member,reply->str, conn->errstr);
