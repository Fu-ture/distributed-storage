...
//默认为localhost
char host_name[HOST_NAME_LEN] = "http://127.0.0.1";
...
void host_init(void) {
    ...
    if(pipe(fd) < 0)
    {
        LOG(DATA_LOG_MODULE, DATA_LOG_PROC, "pipe error");
    }
    pid_t pid;
    if((pid = fork()) < 0) {
        LOG(DATA_LOG_MODULE, DATA_LOG_PROC, "fork error");
    }
    ...
    if(pid == 0) {
        ...
        system("ifconfig eth0 | sed -n '2p' | awk '{ print $2}' | awk -F: '{print $2}'");
    }

    close(fd[1]);
    wait(NULL);
    read(fd[0],host_name,HOST_NAME_LEN);
    close(fd[0]);
